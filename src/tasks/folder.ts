...
export function buildFolderPushCommands(runtime: LoadedRuntime): CommandSpec[] {
  return runtime.config.files.push.map((folder) => ({
    ...
    args: [
      ...
      `--rsync-path=mkdir -p ${path.posix.join(runtime.config.remote.app_path, folder)} && rsync`,
      `${path.resolve(runtime.options.cwd, folder)}/`,
      `${runtime.config.remote.user}@${runtime.config.remote.host}:${path.posix.join(runtime.config.remote.app_path, folder)}/`,
    ],
...
export function buildFolderPullCommands(runtime: LoadedRuntime): CommandSpec[] {
  const mkdirCommands = runtime.config.files.pull.map((folder) => ({
    command: "mkdir",
